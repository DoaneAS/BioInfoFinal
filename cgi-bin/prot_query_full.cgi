...
def make_net_plot(ppi):
    ...
    degree_sequence = sorted(
        nx.degree(G).values(), reverse=True)  # degree sequence
    ...
    plt.savefig('/home/local/CORNELL/asd223/final/images/plot.svg',
...
def make_tables(ppi, network_data):
    ...
    for qp in network_data.keys():
        ...
        line = qp + \
            """ <a href="http://www.ncbi.nlm.nih.gov/gene/%s">(%s)</a>""" % (
                gid, gid)
        poi.append(line)
    poi = ', '.join(poi)
    ...
    for k, v in network_data.items():
        ...
        ps = len(p2s)
        p1idlink = """<a href="http://www.ncbi.nlm.nih.gov/gene/%s">%s</a>""" % (
            p1ann[0], p1ann[0])
...
def all_net():
    con = mdb.connect(
        host='127.0.0.1', user='asd223', passwd='5NKEAP', db='ASD223_db')
    cur = con.cursor()
